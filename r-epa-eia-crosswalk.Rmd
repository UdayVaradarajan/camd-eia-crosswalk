...
eia_860_Early_Release <- "No" # Input "Yes" if early release EIA-860 data should be used
...
current_EIA_860M_month <- "september" # Most recent month in lower-case used to pull EIA-860M
...
refresh_EIA_data <- "Yes" # Input "Yes" here if EIA data should be downloaded and refreshed, "No" if data has already been downloaded
...
eia_860_year <- paste(eia_860_year_num,ifelse(eia_860_Early_Release=="Yes","_Early_Release",""),sep="") # The EIA data year used in ER files
...
for (eia_year in earliest_retirement_year:eia_860_year_num) {
  ...
  unit_code_name = case_when(
          eia_year>=2012 ~ "Unit Code",
          TRUE ~ "UNIT_CODE"
          )
  ...
  eia_gen_year <- # Operating capacity and fuels for historic year
    ...
    select(
        ...
        EIA_UNIT_CODE = .data[[unit_code_name]],
